...
- **Breaking:** Default file location changed from `~/.local/share/todo/todo.db` to `~/.local/share/todo/todo.md`

### Added

- TUI fuzzy filter: press `/` to search items, `ctrl+n`/`ctrl+p` to navigate matches, `enter` to select, `esc` to cancel
- `TODO_FILE` environment variable supports `~` and `$VAR` expansion
...
- TUI renders sections as styled horizontal rules
- TUI `d` on a section deletes it
- TUI `enter` on a section is a no-op (sections can't be toggled)
...
- TUI `d` keybinding now marks unchecked items as done, or deletes checked items
- TUI bulk clean moved from `d` to `c` keybinding

## [2.2.0] - 2026-03-06

### Added

- TUI `space` keybinding to copy the selected item's text to the clipboard

### Changed

- TUI item list no longer displays ID numbers, keeping all items aligned
- TUI `enter` now toggles items (previously both `space` and `enter` toggled)

### Fixed

- TUI items now wrap within the terminal width, with continuation lines indented to align with the text
- TUI text input now stays within terminal bounds and scrolls horizontally
