...
- Replacing `deleteTicket` with `startTicket`/`closeTicket`/`reopenTicket` using `tickets.SetStatus()` is cleaner than the original `tickets.Done()` — all three actions share the same pattern
- Keeping `d` as an alias for `c` (both map to `closeTicket`) preserves backward compatibility with zero extra complexity via `case "d", "c":`
- `tea.ExecProcess(cmd, callback)` returns a `tea.Cmd` directly (not a `func() tea.Msg`), so `editTicket` returns `tea.Cmd` rather than following the `func() tea.Msg` pattern used by other action methods
...
### Iteration 5: Add status management keybindings to TUI
- **Commit**: b94eb72
- **Summary**: Replaced `deleteTicket` method with three new methods (`startTicket`, `closeTicket`, `reopenTicket`) all using `tickets.SetStatus()`. Added `s` (in_progress), `c`/`d` (closed), `r` (open) keybindings in `updateListPanel()`. `d` kept as backward-compatible alias for `c`. Updated status bar hints (`s/start`, `c/close`, `r/reopen`) and help modal entries. Action messages use descriptive prefixes ("Started:", "Closed:", "Reopened:"). All 93 unit tests and 211 bats integration tests pass.

### Iteration 6: Add edit-in-editor action to TUI
- **Commit**: 5610cfe
- **Summary**: Added `editorFinishedMsg` type and `editTicket(id string) tea.Cmd` method that reads `$EDITOR` (defaults to `vi`), constructs file path via `filepath.Join(tickets.DirPath(m.dir), id+".md")`, and uses `tea.ExecProcess(cmd, callback)` to suspend TUI while editing. Added `e` keybinding in `updateListPanel()` guarded on non-empty items. `editorFinishedMsg` handler in `Update()` sets error if present and always reloads tickets. All 93 unit tests and 211 bats integration tests pass.
...
- **Commit**: 7e5da34
- **Summary**: Updated help modal in `renderHelpModal()` to add `e edit in $EDITOR` and `n add note` to Ticket List section, and added new "Views" section with `1`/`2`/`3`/`4` keys between Ticket List and Detail Panel. Updated status bar in `renderStatusBar()` to add `e edit` and `n note` hints in list panel context. Rewrote `cmd/tui.go` Long description with complete keybinding reference covering all panels, ticket actions (a/s/c/d/r/e/n/space), view modes (1-4), and navigation. Removed obsolete references to K/J reorder and old "mark done". All 93 unit tests and 211 bats integration tests pass.
